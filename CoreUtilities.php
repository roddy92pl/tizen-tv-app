...
class CoreUtilities {
	...
	if (stripos($rSource, '|cenc=') !== false) {
		list($sourceUrl, $cencKey) = explode('|cenc=', $rSource, 2);
		$sourceUrl = trim($sourceUrl);
		$cencKey = trim($cencKey);
	}
...
if (!empty($cencKey) && stripos($rStreamSource, '.mpd') !== false) {
    $rFetchOptions .= ' -cenc_decryption_key ' . escapeshellarg($cencKey);
    // UWAGA: nie dodajemy do $rProbeOptions, żeby "Scan source" nie padał
    // $rProbeOptions .= ' -cenc_decryption_key ' . escapeshellarg($cencKey);
}
...
if (isset($rStream['type']) && intval($rStream['type']) === 4) {
